...
// appCredentials are identified by concatenated projectFullName and bundleIdentifier
// userCredentials are identified by id
//
// Expected behaviour of cache (internals)
//
// - when isPrefetched[accountName] true assume everything is synced for that account
// - when credentials[accountName].appCredentials[index] is truthy assume that user and app credentials for that app are synced
...
// - when deleting provisioningProfile modify appCredetnials in cache
...
interface CredentialsCache {
  [accountName: string]: {
    appCredentials: {
      [key: string]: IosAppCredentials;
    };
    userCredentials: {
      [key: string]: IosDistCredentials | IosPushCredentials;
    };
  };
}
...
export default class IosApi {
  ...
  public async createDistCert(
    ...
    // refetching because www might add some fields (certPrivateSigningKey or certSerialNumber)
  ...
  private async refetchAppCredentials(app: AppLookupParams): Promise<void> {
    const { accountName, projectName, bundleIdentifier } = app;
